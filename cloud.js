...
function choose(p,isChild=false){
 child=p;childModeSession=isChild;const parentMode=Boolean(user&&!isChild),profileInitials=initials(p.name);document.body.classList.toggle("parent-child-active",parentMode);const parentBack=$("#parentChildrenBack");if(parentBack)parentBack.hidden=!parentMode;$("#profileName").textContent=p.name;document.querySelector(".top .avatar").textContent=profileInitials;$("#areaProfileName").textContent=p.name;$("#areaAvatar").textContent=profileInitials;
 try{
...
$("#parentChildrenBack")?.addEventListener("click",async()=>{document.body.classList.remove("parent-child-active","school-mode");$("#parentChildrenBack").hidden=true;app(false);parentTab="overview";await profiles();window.scrollTo(0,0)});
