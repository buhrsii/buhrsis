...
function statsMarkupPD(card,data){
 ...
 return `<header class="parent-stats-heading"><span class="eyebrow">${escPD(name)}</span><h2>Statistik & Korrekturen</h2><p>Trage Vergessenes nach oder tippe eine Aktivität an, um sie zu ändern.</p></header><div class="parent-stats-totals"><div><small>XP</small><b>${escPD((card.querySelector(".parent-child-head>span")?.textContent||"0").replace(/[^0-9]/g,""))}</b></div><div><small>7 TAGE LERNEN</small><b>${totals.learning} Min.</b></div><div><small>7 TAGE PUTZEN</small><b>${totals.brushing}×</b></div></div><section class="parent-add-section"><h3>Aktivität hinzufügen</h3><div class="parent-add-actions"><button type="button" data-add-activity="learning"><span>📚</span>Lernzeit</button><button type="button" data-add-activity="homework"><span>📝</span>Hausaufgaben</button><button type="button" data-add-activity="brush"><span>🪥</span>Zähneputzen</button></div></section><div class="parent-correction-editor" data-correction-editor hidden></div><div class="parent-filter-tabs" role="tablist"><button class="active" type="button" data-activity-filter="all">ALLE</button><button type="button" data-activity-filter="learning">LERNEN</button><button type="button" data-activity-filter="homework">HAUSAUFGABEN</button><button type="button" data-activity-filter="brush">ZÄHNE</button></div><div class="parent-activity-rows">${rows.length?rows.map(row=>`<div data-activity-kind="${row.kind}">${activityButtonPD(row,data)}</div>`).join(""):'<p class="parent-activity-empty">Noch keine Aktivitäten gespeichert.</p>'}</div>`;
}
...
function manualTimerEditorPD(type,data){
 const end=new Date(),start=new Date(end.getTime()-30*60000),learning=type==="learning",subjects=(data.snapshot.subjects||[]);
 return `<form data-manual-activity="${type}" class="parent-edit-form"><div><span>${learning?"📚":"📝"}</span><h3>${learning?"Lernzeit hinzufügen":"Hausaufgaben hinzufügen"}</h3><p>Die XP werden aus der eingetragenen Zeit berechnet.</p></div><label>Fach<select name="subject" required><option value="">Fach auswählen</option>${subjects.map(subject=>`<option value="${subject.id}">${escPD(subject.name)}</option>`).join("")}</select></label>${learning?"":'<label>Aufgabe<input name="title" maxlength="80" placeholder="z. B. Mathe Arbeitsblatt"></label>'}<label>Start<input name="started" type="datetime-local" value="${localInputPD(start)}" required></label><label>Ende<input name="ended" type="datetime-local" value="${localInputPD(end)}" required></label><button type="submit">AKTIVITÄT HINZUFÜGEN</button><small>Erlaubt sind 5 bis 240 Minuten.</small><p data-edit-message></p></form>`;
}
...
function updateCardXpPD(card,xp){const badge=card.querySelector(".parent-child-head>span");if(badge&&Number.isFinite(Number(xp)))badge.textContent=`${Number(xp)} XP`}
...
function bindStatsPD(card,data){
 ...
 content.querySelectorAll("[data-add-activity]").forEach(button=>button.onclick=()=>{const type=button.dataset.addActivity;editor.hidden=false;editor.innerHTML=type==="brush"?brushEditorPD(todayISO(),data):manualTimerEditorPD(type,data);editor.scrollIntoView({behavior:"smooth",block:"start"});const form=editor.querySelector("form");if(type==="brush"){form.onsubmit=async event=>{event.preventDefault();const submit=event.submitter,message=form.querySelector("[data-edit-message]"),values=Object.fromEntries(new FormData(form));submit.disabled=true;message.textContent="Wird gespeichert …";try{await saveBrushCorrectionPD(card,values.date,Number(values.count));await refreshStatsPD(card)}catch(error){message.textContent="Speichern fehlgeschlagen: "+error.message;submit.disabled=false}};return}form.onsubmit=async event=>{event.preventDefault();const submit=event.submitter,message=form.querySelector("[data-edit-message]"),values=Object.fromEntries(new FormData(form));submit.disabled=true;message.textContent="Wird hinzugefügt …";try{const client=await clientPD(),result=await client.rpc("buhrsi_parent_add_activity",{p_child:card.dataset.childCard,p_timer_type:type,p_subject:values.subject,p_started_at:new Date(values.started).toISOString(),p_ended_at:new Date(values.ended).toISOString(),p_title:values.title||null});if(result.error)throw result.error;updateCardXpPD(card,result.data?.xp);await refreshStatsPD(card)}catch(error){message.textContent=error.message;submit.disabled=false}}});
