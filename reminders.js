...
const VAPID_PUBLIC_KEY="BPHcQhAlI0Z7TnJbkrCh5eYs0z94eew0SkgOWh_lgOX1o9duQDreA_zAP3NSqivM5P_kGBa5nxqUuqx_ZIwaUr4";
...
function mount(){
  ...
  const social=document.getElementById("social027")||document.getElementById("homeSection");
  if(!social)return;
  const section=document.createElement("section");section.id="reminders028";section.className="reminders028";
  section.innerHTML=`
    <div class="reminders028-head"><div><span class="eyebrow">ERINNERUNGEN</span><h2>Zahnputz-Zeiten</h2></div><span>🔔</span></div>
    <div class="reminders028-grid">
      <div class="reminders028-row"><input id="morningEnabled028" type="checkbox"><div><label for="morningTime028">Morgens</label><input id="morningTime028" type="time" value="07:00"></div></div>
      <div class="reminders028-row"><input id="eveningEnabled028" type="checkbox"><div><label for="eveningTime028">Abends</label><input id="eveningTime028" type="time" value="19:00"></div></div>
    </div>
    <div class="reminders028-actions"><button id="saveReminders028" class="primary028" type="button">ZEITEN SPEICHERN</button><button id="enablePush028" class="secondary028" type="button">BENACHRICHTIGUNGEN AKTIVIEREN</button></div>
    <p id="reminderStatus028" class="reminders028-status"></p>
    <p class="reminders028-note">Wenn für den jeweiligen Zeitraum schon geputzt wurde, kommt keine Erinnerung mehr.</p>`;
...
async function saveSettings(){
  ...
  const {error}=await reminderSb.rpc("buhrsi_reminders_save",args({
    p_morning_enabled:document.getElementById("morningEnabled028").checked,
    p_morning_time:document.getElementById("morningTime028").value,
    p_evening_enabled:document.getElementById("eveningEnabled028").checked,
    p_evening_time:document.getElementById("eveningTime028").value,
    p_timezone:tz
  }));
...
async function enablePush(){
  ...
  if(permission!=="granted")return setStatus("Benachrichtigungen wurden nicht erlaubt.");
  ...
  try{
    ...
    const {error}=await reminderSb.rpc("buhrsi_push_upsert",args({
    ...
    await saveSettings();
    setStatus("Benachrichtigungen sind auf diesem Gerät aktiviert ✓");
  }catch(e){console.error(e);setStatus("Aktivierung fehlgeschlagen: "+(e.message||e))}
}

function selectChild(profile){
  currentChild=profile||readChild();currentPin=readPin();
  if(currentChild){mount();loadSettings()}
}
...
try{
  ...
  reminderSb=m.createClient(SUPABASE_URL,SUPABASE_KEY,{auth:{persistSession:true,autoRefreshToken:true,detectSessionInUrl:false}});
  selectChild(readChild());
}catch(e){console.error("Erinnerungen konnten nicht geladen werden",e)}
